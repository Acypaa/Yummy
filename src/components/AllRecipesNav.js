...
import RecipeList from './RecipeList';  // Импортируем RecipeList
...
const AllRecipesNav = ({ addRecipe, handleSearch, onFilterChange, onApplyFilters, onClearFilters, selectedCuisine }) => {
  ...
  const { recipe } = useContext(Context); // Используем контекст для доступа к данным
  ...
  const handleDelete = (id) => {
    const storedRecipes = JSON.parse(localStorage.getItem('recipes')) || [];
    const updatedRecipes = storedRecipes.filter(recipe => recipe.id !== id || recipe.author);
    ...
    <div className='AllRecipesNav'>
      ...
      >
        <div className="modal__content">
          ...
          <select 
            name="dishType" 
            onChange={(e) => setNewRecipe({ ...newRecipe, dishType: e.target.value })}
          >
